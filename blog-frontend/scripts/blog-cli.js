...
async function syncTags() {
  ...
  for (const file of files) {
    const { data, content } = matter(
      fs.readFileSync(path.join(BLOGS_DIR, file), "utf8"),
    );
...
async function main() {
  ...
  if (action === "Create Post") {
    ...
    const metadata = {
      ...
      blogNumber:
        fs.readdirSync(BLOGS_DIR).filter((f) => f.endsWith(".md")).length + 1,
    };
    ...
    if (files.length === 0) {
      ...
      files.forEach((f) => {
        const { data } = matter(
          fs.readFileSync(path.join(BLOGS_DIR, f), "utf8"),
        );
        console.log(
          `${data.publish ? "✅" : "📝"} ${data.title.padEnd(30)} | ${data.slug}`,
        );
      });
      console.log("------------------\n");
    }
    ...
    consola.success(
      `Published ${toPublish} (Reading time: ${data.readingTime})`,
    );
